...
def post_init_hook(cr, registry):
    ...
    def create_sequence_service(cr, registry):
        ...
        for company in companies:
            for data in sequence_data:
                # Check if sequence already exists for this company and code
                existing_sequence = env['ir.sequence'].search([
                    ('code', '=', data['code']),
    ...
    def create_service_type(cr, registry):
        ...
        for company in companies:
            for item in types:
                # Check if service type already exists for this company
                existing_type = env['fleet.service.type'].search([
                    ('name', '=', item['name']),
    ...
    def create_fleet_setting(cr, registry):
        ...
        for company in companies:
            # Check if fleet configuration already exists for this company
            existing_config = env['fleet.configuration.service'].search([
                ('company_id', '=', company.id)
            ], limit=1)
