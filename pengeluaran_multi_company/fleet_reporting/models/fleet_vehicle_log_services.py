		...
		if self.is_service:
			...
			for line in self.list_sparepart:
				...
				if line.available_qty < line.product_qty:
					raise ValidationError('Qty Produk ' + str(line.product_id.name) + " lebih sedikit dari yang tersedia.")
...
class ProductLine(models.Model):
	...
	product_id = fields.Many2one('product.product', copy=False, domain="[('qty_available', '>', 0)]")
